...
public class MainActivity extends Activity {
    ...
    private void showGraphicalConnectionState() {
        ...
        if (iMountedConnections == 0) {
            image_view.setImageResource(R.drawable.none_connected);
        } else if (iMountedConnections == iEnabledConnections) {
            image_view.setImageResource(R.drawable.all_connected);
        } else {
            image_view.setImageResource(R.drawable.any_connected);
        }
    }
    ...
    private void initializeMountPointElementCallback(Intent intent) {
        if (intent.getBooleanExtra(MainService.ACTION_GET_MOUNT_STATES_ANSWER_IS_FIRST_LIST_ELEMENT, false)) {
            iMountedConnections = iEnabledConnections = 0;
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        } else if (id == R.id.edit_mountpoints_menuitem) {
            startActivityForResult(new Intent(this, MountPointsEdition.class), 1);
        }
        return true;
    }
    ...
    protected void onActivityResult(int request_code, int result_code, Intent intent) {
        if (request_code == 0) {
            MainService.reloadPreferences(this);
        }
    }
}
