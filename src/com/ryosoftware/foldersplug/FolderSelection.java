...
public class FolderSelection extends Activity implements OnClickListener, OnItemClickListener, CreateFolderCallback {
    ...
    public void onCreate(Bundle saved_instance_state) {
        ...
        if ((iCurrentPath == null) || (iCurrentPath.equals("")) || (!iSuperuserCommandsExecutor.isFolder(iCurrentPath))) {
            iCurrentPath = ROOT_DIRECTORY;
        } else if (iCurrentPath.equals("/")) {
            iCurrentPath = "";
        }
    ...
    public void onResume() {
        super.onResume();
        if (!iSuperuserCommandsExecutor.canRunRootCommands()) {
            DialogUtilities.showAlertDialog(this,
                    R.string.initialization_error_possibly_cause_no_root_adquired,
    ...
    public void onCreateFolderDialogEnded(int result_code, Intent intent) {
        if (result_code == RESULT_OK) {
            ...
            if (!iSuperuserCommandsExecutor.createFolder(String.format("%s/%s", iCurrentPath, filename))) {
                DialogUtilities.showAlertDialog(this, R.string.cannot_create_folder, null);
            } else {
                iCurrentPath = String.format("%s/%s", iCurrentPath, filename);
            }
            showDirectoryContents(iCurrentPath);
        }
    }
    ...
    private void showDirectoryContents(String pathname, ArrayList<String> childs) {
        ...
        if (childs != null) {
            ...
            for (int i = 0; i < childs.size(); i++) {
                iFilesList.add(createListViewItem(childs.get(i), R.drawable.folder));
            }
        }
    ...
    private void showDirectoryContents(String pathname, boolean inhibite_warnings) {
        ...
        if (childs == null) {
            while (true) {
                ...
                if ((childs != null) || (pathname.equals(""))) {
                    break;
                }
            }
            if (!inhibite_warnings) {
                DialogUtilities.showAlertDialog(this, R.string.cannot_read_folder,
                        new CannotReadFolderDialogButtonClickCallback(pathname, childs));
                show_directory_contents = false;
            }
        }
        if (show_directory_contents) {
            showDirectoryContents(pathname, childs);
        }
    }
    ...
    private class FolderNotExistsDialogButtonClickCallback implements ButtonClickCallback {
        ...
        public void onClick() {
            while ((!iSuperuserCommandsExecutor.isFolder(iPathname)) && (!iPathname.equals(""))) {
                iPathname = getParent(iPathname);
            }
    ...
    private void showDirectoryContents(String pathname) {
        ...
        if (show_directory_contents) {
            showDirectoryContents(pathname, false);
        }
    }
    ...
    public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
        ...
        if ((!iCurrentPath.equals("")) && (position == 0)) {
