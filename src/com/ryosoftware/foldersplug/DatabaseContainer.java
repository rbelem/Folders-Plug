...
public class DatabaseContainer {
    ...
    public void open() throws SQLException {
        ...
        if (iSQLiteDatabase == null) {
            ...
            Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, "Database is already opened");
        }
    }
    ...
    public long getRowIdBySource(String source) {
        ...
        if (id == ROW_ID_ERROR) {
            Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, String.format("Id for row with source='%s' couldn't be retrieved", source));
        } else {
            Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, String.format("Retrieved id for row with source='%s' is %d", source, id));
        }
        return id;
    }
}
