...
public class MainService extends Service {
    ...
    private class MainServiceBroadcastReceiver extends BroadcastReceiver {
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(ACTION_RELOAD_PREFERENCES)) {
                reloadPreferences(intent);
            } else if (action.equals(ACTION_GET_MOUNT_STATES)) {
                getMountPointsStates(intent);
            } else if (action.equals(ACTION_UPDATE_MOUNTPOINT)) {
                updateMountPoint(intent);
            } else if (action.equals(ACTION_REMOVE_MOUNTPOINT)) {
                removeMountPoint(intent);
            } else if (action.equals(ACTION_REMOVE_MOUNTPOINTS)) {
                removeMountPoints(intent);
            } else if (action.equals(ACTION_DEVICE_STATE_CHANGED)) {
                deviceStateChanged(intent);
            }
        }
    }
    ...
    private void executeMountPointCommands(MountPoint mountpoint, boolean mount) {
        ...
        if (mountpoint.getMounted() != mount) {
            ...
            Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, "Mount state is correct");
        }
    }
    ...
    private void deviceBootCompleted() {
        ...
        if (Constants.getPreferences(this).getBoolean(Constants.MOUNT_ON_BOOT_PREFERENCE_KEY, Constants.MOUNT_ON_BOOT_PREFERENCE_DEFAULT)) {
            ...
            Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, "Automounts when device boots is disabled");
        }
    }
